...
- Host가 생성한 실제 투사체는 발사 무기 ID, 발사 Tick, 발사 Sequence, 투사체 순번으로 이루어진
  ...
  발사 순간의 무기 소유자를 `PredictionOwner`로 함께 고정해 로컬 발사자를 식별한다. 따라서 실제체가
  도착하기 전에 무기를 드롭하거나 다른 플레이어에게 넘겨도 예측 표시의 소유자가 바뀌지 않는다.
  발사 Client는 실제체의 보간 스냅샷이 준비되면
  ...
  최대 3 Tick 동안 재시도해 매칭 실패를 원격 투사체로 오판한 한 프레임 노출을 막는다. 기한 안에
  무기나 예측체를 찾지 못하면 남아 있는 같은 키의 예측체를 반환하고 실제체를 표시해 영구히 숨지 않게
  한다. 연결된 실제체가 Despawn될 때 예측체를 풀에 반환한다.
