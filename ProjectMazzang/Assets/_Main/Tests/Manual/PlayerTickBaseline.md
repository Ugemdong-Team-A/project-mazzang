...
- 비-Host Client가 무기를 장착하면 해당 무기의 Input Authority가 장착자에게 전달되고, 드롭하면
  제거된다. 실제 투사체가 도착하면 같은 예측 키의 로컬 표시가 풀로 돌아가며 두 탄이 겹치지 않는다.
- 예측 투사체는 실제 투사체 프리팹과 같은 Sprite, 원본 크기, Trail 설정을 사용하고 충돌·피해·패링
  판정에는 참여하지 않는다. 무기 프리팹에는 별도 예측 프리팹 참조가 필요하지 않다.
