...
namespace ProjectMazzang.Tests
{
    public sealed class PlayerBaselineContractTests
    {
        ...
        [Test]
        public void PresentationComponents_StayOutsideTickPipeline()
        {
            Type componentType =
                GetRuntimeType(
                    "PlayerComponent");
                     ...
                     })
            {
                ...
                Assert.That(
                    componentType.IsAssignableFrom(
                        presentationType),
                    Is.True,
                    $"{typeName}은 PlayerController의 Present 관리 대상이어야 합니다.");

                Assert.That(
                    tickModuleType.IsAssignableFrom(
                        presentationType),
                    Is.False,
                    $"{typeName}이 Tick 시뮬레이션에 참여하고 있습니다.");
            }
            ...
            FieldInfo modulesField =
                controllerType.GetField(
                    "_modules",
                    BindingFlags.Instance |
                    BindingFlags.NonPublic);

            FieldInfo tickModulesField =
                controllerType.GetField(
                    "_tickModules",
                    BindingFlags.Instance |
                    BindingFlags.NonPublic);
            ...
            Assert.That(
                modulesField?.FieldType,
                Is.EqualTo(
                    componentType.MakeArrayType()));

            Assert.That(
                tickModulesField?.FieldType,
                Is.EqualTo(
                    tickModuleType.MakeArrayType()));
        }
